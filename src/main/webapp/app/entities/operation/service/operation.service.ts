...
export class OperationService {
  ...
  queryHistory(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IOperation[]>(`${this.resourceUrl}/operationHistory`, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient(operation: IOperation): IOperation {
    return Object.assign({}, operation, {
      date: operation.date?.isValid() ? operation.date.format(DATE_FORMAT) : undefined,
    });
  }
