...
export class VirementHistoryComponent implements OnInit {
  ...
  constructor(protected virementService: VirementService, protected modalService: NgbModal, protected parseLinks: ParseLinks) {
    this.virements = [];
    ...
    this.virementService
      ...
      .subscribe({
        next: (res: HttpResponse<IVirement[]>) => {
          ...
          this.paginateVirements(res.body, res.headers);
        },
        error: () => {
          this.isLoading = false;
        },
  ...
  protected paginateVirements(data: IVirement[] | null, headers: HttpHeaders): void {
